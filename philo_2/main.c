...
/*   Updated: 2024/05/07 14:45:43 by yeoshin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	info(int ac, char *av[])
{
	...
	philo = init_philo(fork, ft_atoi(av[1]), info->mutex_fork, info);
	if (philo == NULL)
...
static int	thread_start(pthread_t *threads, t_info *info, t_philo **philo)
{
	...
	while (idx < info->philo_count)
	{
		...
		ret = pthread_create(&threads[idx], NULL, thread_function, info);
		if (ret != 0)
		{
